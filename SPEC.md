...
- **Armadilha da T38 — `.fixedSize()` para corrigir wrap estoura a tela em Dynamic Type grande.** Usado para impedir que "Julho" quebrasse em "Jul"/"ho" dentro da pill, o `.fixedSize()` fez a linha do ramo aniversário ultrapassar a borda direita do device em `accessibility-extra-extra-extra-large`, deixando o stepper do dia **cortado e inalcançável** (o `ScrollView` é só vertical). Trocou bug visual por bug funcional. **Lição:** `.fixedSize()` em conteúdo com texto localizável precisa ser verificado nas categorias de acessibilidade do Dynamic Type; aceitar quebra de linha em fonte gigante é preferível a tornar um controle inacessível.
