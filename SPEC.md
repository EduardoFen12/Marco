...
- [ ] **Achado do `spec-reviewer` na T40 — `DateType.symbolName` duplicado e divergente entre alvos.** A T40 criou o mapeamento símbolo↔categoria em `DateType` (`heart`/`star`/`leaf`, `Marco/Views/ImportantDateListView.swift`) partindo da premissa, escrita na task, de que "não existe mapeamento símbolo↔categoria hoje". Existia um: `MarcoWidgets/NextDateWidget.swift` já tinha uma `private extension DateType { var symbolName }` própria, com ícones **diferentes** (`birthday.cake`/`star`/`flame`). Como `MarcoWidgets` é outro módulo e a extensão de lá é `private`, os dois convivem sem conflito de compilação — mas app e widget mostram ícones diferentes para a mesma categoria. Consolidar movendo o mapeamento para `Shared/DateType.swift` (visível aos dois alvos, como `stripeColor` **não** é hoje) se/quando a inconsistência visual incomodar.
- **Passo manual recorrente da Fase 3 — extração do String Catalog só acontece pelo Xcode.app.** `xcodebuild build` pela CLI não extrai chaves novas para `Marco/Localizable.xcstrings`; as strings introduzidas por T37–T40 entraram no catálogo em commits separados feitos após abrir o projeto no Xcode (ver `175a3b8`). Pendente para a T40: `"Voltar"`, `"SUGERIR PRESENTE"`, `"GERAR MENSAGEM"`, `"DIAS"`, `"Sugestão de presente"`, `"Mensagem sugerida"`. Sem impacto funcional em pt-BR (a chave é a própria string base), só na completude da localização.
