...
- [ ] **Teste da T30 vermelho sob execução paralela — `photoDataFazRoundTripAoSalvarERecarregar`.** Confirmado independentemente por três revisões (reproduz no commit `368e822`, antes de T31/T32): o teste falha com `reloaded.photoData → nil` quando a suíte `MarcoTests` roda inteira (paralelismo padrão do Swift Testing) e passa isolado ou com `-parallel-testing-enabled NO`. É o único teste da suíte que grava `photoData` (`@Attribute(.externalStorage)`); hipótese em investigação é que os `ModelConfiguration(isStoredInMemoryOnly: true)` de todos os testes compartilham a mesma URL de store e, por consequência, o diretório de blobs externos, com containers concorrentes stompando o blob uns dos outros. Correção delegada — atualizar esta nota com a causa raiz confirmada.
